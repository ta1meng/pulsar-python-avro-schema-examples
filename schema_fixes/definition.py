...
class Record(with_metaclass(RecordMeta, object)):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for k, value in self._fields.items():
            if k in kwargs:
                ...
                # Convert dictionary object into a Record object
                if isinstance(kwargs[k], dict):
                    kwargs[k] = type(getattr(self, k))(**kwargs[k])
    ...
    def schema(cls):
        ...
        if hasattr(cls, '_namespace'):
            schema['namespace'] = cls._namespace
        schema['fields'] = []
        if hasattr(cls, '_doc'):
            schema['doc'] = cls._doc
        ...
        for name in cls._fields.keys():
            ...
            if hasattr(field, '_default') and field._default != None:
                field_body['default'] = field._default
            if hasattr(field, '_doc') and field._doc != None:
                field_body['doc'] = field._doc
            schema['fields'].append(field_body)
        return schema
...
class Field(object):
    def __init__(self, default=None, required=False, doc=None):
